...
def main():
    ...
    executionCount = 0
    isHalt = False  # เพิ่มตัวแปร isHalt เริ่มต้นเป็น False

    if len(sys.argv) != 2:
        print("error: usage: python simulator.py <machine-code file>")
        exit(1)

    machine1 = sys.argv[1]

    try:
        with open(machine1, "r") as filePtr:
        ...
        abortWithError(f"Can't open file {machine1}")
    
    # Print initial state
    printState(state)
    
    while not isHalt:  # ใช้ตัวแปร isHalt ในการควบคุมการหยุดการทำงานของ loop
